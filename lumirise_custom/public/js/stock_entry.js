...
frappe.ui.form.on("Stock Entry", {
	...
	refresh(frm) {
								...
								.then(async (r) => {
									...
									const row = (frm.doc.items || []).find(
										(d) =>
											...
											d.s_warehouse === p.current_warehouse &&
											!d.lr_rm_package
									);
									if (!row) {
										frappe.throw(
											__("No unscanned row for item {0} from {1}.", [
												p.item_code,
												p.current_warehouse,
											])
										);
		...
		if (
			frm.doc.docstatus === 1 &&
			frm.doc.stock_entry_type === "Material Issue to Shop Floor"
		) {
			frm.add_custom_button(
				__("Acknowledge Receipt (Material Receipt)"),
				() => {
