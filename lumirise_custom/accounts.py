...
* The rejected qty is read from the Purchase Receipt(s) the invoice was made from
  ...
  rejected stock already sits in the rejection warehouse (handled at GRN time), so
  this debit note is FINANCIAL only (update_stock = 0): it nets the payable down.
...
def _auto_debit_note(doc):
	...
	dn.remarks = (
		...
		f"PENDING APPROVAL — review and submit to net the payable."
	)
	...
	try:
		...
		create_task(
			...
			description=(
				...
				f"Verify and submit it so the vendor is debited for the rejection."
			),
	...
	frappe.msgprint(
		...
		f"It is pending approval — open and submit it to debit the vendor.",
